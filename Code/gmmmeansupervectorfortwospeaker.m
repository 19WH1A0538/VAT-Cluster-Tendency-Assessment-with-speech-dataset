...
config1='D:/HTK-3.4.1/htk/config';
  for i= 1: No_of_Utterances
    %  for j=1:No_of_Utterances
         mySourceFile = ['D:/speechdatanew/twospeaker/' int2str(i) '.wav'];
         myTargetFile = ['D:/speechdatanew/twospeaker/' int2str(i) '.mfc'];
         rv= system([get_htk_path 'HCopy ' args  config1 ' '  mySourceFile ' '  myTargetFile]);
%   %end
  end
   fid = fopen( 'twospeaker.txt', 'wt' );
for t=1:80
    mySourceFile = ['D:\speechdatanew\twospeaker\' int2str(t) '.mfc'];
     fprintf( fid, '%s\n',mySourceFile );
end
%     args=['-A -D -T 1 -C ']; 
%      config1='D:/HTK-3.4.1/htk/config';
...
%  
    ...
    A3='twospeaker.txt';
          ...
          for i= 1: 80
                     fid = fopen( 'twospeaker.txt', 'wt' );
                         targetpath=['D:\speechdatanew\twospeaker\' int2str(i) '.mfc'];
                         fprintf( fid, '%s\n',targetpath);
