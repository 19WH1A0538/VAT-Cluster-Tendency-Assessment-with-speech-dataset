...
%   No_of_Speakers=4;
  No_of_Utterances=140;
args=['-A -D -T 1 -C ']; 
config1='D:/HTK-3.4.1/htk/config';
  for i= 1: No_of_Utterances
    %  for j=1:No_of_Utterances
         mySourceFile = ['D:/speechdatanew/fourspeaker/' int2str(i) '.wav'];
         myTargetFile = ['D:/speechdatanew/fourspeaker/' int2str(i) '.mfc'];
         rv= system([get_htk_path 'HCopy ' args  config1 ' '  mySourceFile ' '  myTargetFile]);
%   %end
  end
   fid = fopen( 'fourspeaker.txt', 'wt' );
for t=1:140
    mySourceFile = ['D:\speechdatanew\fourspeaker\' int2str(t) '.mfc'];
     fprintf( fid, '%s\n',mySourceFile );
end
%     args=['-A -D -T 1 -C ']; 
%      config1='D:/HTK-3.4.1/htk/config';
...
%  
    ...
    A3='fourspeaker.txt';
          ...
          for i= 1: 140
                     fid = fopen( 'fourspeaker.txt', 'wt' );
                         targetpath=['D:\speechdatanew\fourspeaker\' int2str(i) '.mfc'];
                         fprintf( fid, '%s\n',targetpath);
